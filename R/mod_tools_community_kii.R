...
mod_tools_community_kii_ui <- function(id) {
  ns <- NS(id)
  tagList(
...
#'
#' @noRd
mod_tools_community_kii_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    output$tool_present <- shiny::renderText({
      if (isTRUE(protocol_r()$.tool_community_kii)) "true" else "false"
    })
    ...
    all_indicators <- shiny::reactive({
      ...
      bank <- protocol_r()$framework$modified_indicator_bank[
        protocol_r()$framework$modified_indicator_bank$tool == "kii_community",
    ...
    output$summary_table <- shiny::renderTable({
      ...
      survey <- protocol_r()$tools$tool_kii_community_iphra_v2$revised_survey
    ...
    observeEvent(input$sector_filter, {
      ...
      protocol_r()$set(field = "tools",
                       role = "tool_kii_community_iphra_v2",
    ...
    observeEvent(session$userData$flags$project_loaded, {
      ...
      tool  <- proto$tools[["tool_kii_community_iphra_v2"]]
    ...
    observeEvent(input$sector_filter, {
      iphra_save_tool_field(
        protocol_r()$tools[["tool_kii_community_iphra_v2"]],
    ...
    observeEvent(input$selected, {
      phrutils::phr_try({
        ...
        tool <- protocol_r()$tools[["tool_kii_community_iphra_v2"]]
        if (!is.null(tool)) {
          tool$set(
        ...
        protocol_r()$tools$tool_kii_community_iphra_v2$filter_survey_by_indicator(indicator_codes = indicators_selected$indicator_code)
        ...
        phrutils::phr_message(
          ...
          origin = phrutils::phr_txt("Community KII Tool: Selection Sync")
        )
      },
      on_error = "warn",
      origin = phrutils::phr_txt("Community KII Tool: Selection Sync"),
    ...
    observeEvent(input$preset_core_kii, {
      phrutils::phr_try({
        ...
        result <- phrutils::phr_try_step({
          phrutils::phr_message(
          phrutils::phr_txt("KII core preset selection completed."),
          origin = phrutils::phr_txt("KII Tool: Preset Core")
        )
    ...
    observeEvent(input$preset_full_kii, {
      phrutils::phr_try({
        ...
        result <- phrutils::phr_try_step({
          ...
          origin = phrutils::phr_txt("KII Tool: Preset Full")
        )
        ...
        phrutils::phr_message(
          phrutils::phr_txt("Full KII preset applied successfully."),
          origin = phrutils::phr_txt("KII Tool: Preset Full")
        )
        ...
        result <- phrutils::phr_try_step({
          phrutils::phr_message(
          phrutils::phr_txt("Full KII preset selection completed."),
          origin = phrutils::phr_txt("KII Tool: Preset Full")
        )
    ...
    observeEvent(input$export_tool, {
      phrutils::phr_try({
        if (!isTRUE(protocol_r()$.tool_community_kii)) {
          shiny::showModal(shiny::modalDialog(
            title = phrutils::phr_txt("Export Community KII Tool"),
            phrutils::phr_txt("The Community Key Informant tool has not been added to the protocol yet. Please add it from the Tool Design page before exporting."),
        ...
        shiny::showModal(shiny::modalDialog(
          title = phrutils::phr_txt("Export Community KII Tool"),
          shiny::tagList(
            shiny::p(phrutils::phr_txt("Click below to save the Community Key Informant tool as an Excel workbook with three sheets: revised_survey, revised_choices, and revised_settings.")),
      ...
      hint = phrutils::phr_txt("Ensure the Community Key Informant tool has been added to the protocol and exposes revised_survey / revised_choices / revised_settings.")
      )
    })

    output$download_tool <- shiny::downloadHandler(
      filename = function() {
        paste0("tool_kii_community_iphra_v2",
      ...
      content = function(file) {
        ...
        sheets <- list(
          survey   = safe_df(protocol_r()$tools$tool_kii_community_iphra_v2$revised_survey),
          choices  = safe_df(protocol_r()$tools$tool_kii_community_iphra_v2$revised_choices),
          settings = safe_df(protocol_r()$tools$tool_kii_community_iphra_v2$revised_settings)
        )
