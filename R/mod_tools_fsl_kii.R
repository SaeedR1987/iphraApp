...
mod_tools_fsl_kii_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(input$selected, {
      ...
      on_error = "warn",
      origin = iphra_txt("FSL Service Provider KII Tool: Selection Sync"),
      ...
      iphra_try({
        if (!isTRUE(protocol_r()$.tool_fsl_provider_kii)) {
          shiny::showModal(shiny::modalDialog(
            title = iphra_txt("Export FSL KII Tool"),
            iphra_txt("The FSL Service Provider KII tool has not been added to the protocol yet. Please add it from the Tool Design page before exporting."),
        ...
        shiny::showModal(shiny::modalDialog(
          title = iphra_txt("Export FSL KII Tool"),
          shiny::tagList(
            shiny::p(iphra_txt("Click below to save the FSL Service Provider KII tool as an Excel workbook with three sheets: revised_survey, revised_choices, and revised_settings.")),
    ...
    output$download_tool <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        iphra_message(
          iphra_txt("FSL KII tool exported to Excel."),
          origin = iphra_txt("FSL KII Tool: Export")
        )
      }
    )
