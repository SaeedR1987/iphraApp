...
mod_tools_fsl_kii_ui <- function(id) {
  ns <- NS(id)
  tagList(
    ...
    shiny::conditionalPanel(
      ...
      shiny::fluidRow(
        shinydashboard::box(
          ...
          shiny::actionButton(ns("preset_core"), "Core FSL Service Provider KII"),
          shiny::actionButton(ns("preset_full"), "Full FSL Service Provider KII"),
...
#' tools_fsl_kii Server Functions
#'
#' @noRd
mod_tools_fsl_kii_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    output$tool_present <- shiny::renderText({
      if (isTRUE(protocol_r()$.tool_fsl_provider_kii)) "true" else "false"
    })
    ...
    all_indicators <- shiny::reactive({
      ...
      bank <- protocol_r()$framework$modified_indicator_bank[
        protocol_r()$framework$modified_indicator_bank$tool == "kii_fsl_provider",
    ...
    output$summary_table <- shiny::renderTable({
      ...
      survey <- protocol_r()$tools$tool_kii_fsl_service_provider_iphra_v2$revised_survey
    ...
    observeEvent(input$sector_filter, {
      ...
      protocol_r()$set(field = "tools",
                       role = "tool_kii_fsl_service_provider_iphra_v2",
    ...
    observeEvent(session$userData$flags$project_loaded, {
      ...
      tool  <- proto$tools[["tool_kii_fsl_service_provider_iphra_v2"]]
    ...
    observeEvent(input$sector_filter, {
      iphra_save_tool_field(
        protocol_r()$tools[["tool_kii_fsl_service_provider_iphra_v2"]],
    ...
    observeEvent(input$selected, {
      phrutils::phr_try({
        ...
        tool <- protocol_r()$tools[["tool_kii_fsl_service_provider_iphra_v2"]]
        if (!is.null(tool)) {
          tool$set(
        ...
        protocol_r()$tools$tool_kii_fsl_service_provider_iphra_v2$filter_survey_by_indicator(indicator_codes = indicators_selected$indicator_code)
        ...
        phrutils::phr_message(
          ...
          origin = phrutils::phr_txt("FSL KII Tool: Selection Sync")
        )
      },
      on_error = "warn",
      origin = phrutils::phr_txt("FSL Service Provider KII Tool: Selection Sync"),
    ...
    observeEvent(input$preset_obj_fsl_kii, {
      phrutils::phr_try({
        ...
        result <- phrutils::phr_try_step({
          ...
          phrutils::phr_message(
            phrutils::phr_txt("FSL KII objectives preset applied successfully."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Objectives")
          )
        ...
        result <- phrutils::phr_try_step({
          ...
          phrutils::phr_message(
            phrutils::phr_txt("FSL KII objectives preset selection completed."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Objectives")
          )
    ...
    observeEvent(input$preset_core_obs, {
      phrutils::phr_try({
        ...
        result <- phrutils::phr_try_step({
          phrutils::phr_message(
            phrutils::phr_txt("Validation checks passed (dummy mode)."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Core")
          )
        ...
        result <- phrutils::phr_try_step({
          ...
          phrutils::phr_message(
            phrutils::phr_txt("FSL KII core preset applied successfully."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Core")
          )
        ...
        }, step = "mod_tools_fsl_kii_server/observeEvent_preset_core_fsl_kii/Core Logic")
        if (phrutils::phr_failed(result)) return(result)
        ...
        result <- phrutils::phr_try_step({
          phrutils::phr_message(
            phrutils::phr_txt("FSL KII core preset selection completed."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Core")
          )
        }, step = "mod_tools_fsl_kii_server/observeEvent_preset_core_fsl_kii/Result Handling")
        if (phrutils::phr_failed(result)) return(result)
    ...
    observeEvent(input$preset_full_obs, {
      phrutils::phr_try({
        ...
        result <- phrutils::phr_try_step({
          phrutils::phr_message(
            phrutils::phr_txt("Validation checks passed (dummy mode)."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Full")
          )
        }, step = "mod_tools_fsl_kii_server/observeEvent_preset_full_fsl_kii/Validation")
        if (phrutils::phr_failed(result)) return(result)
        ...
        result <- phrutils::phr_try_step({
          ...
          phrutils::phr_message(
            phrutils::phr_txt("Full FSL KII preset applied successfully."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Full")
          )

          # --- Future: sync with project session-level KII data ---
          # session$userData$project$set_selection("kii_full", selected_kii())
        }, step = "mod_tools_fsl_kii_server/observeEvent_preset_full_fsl_kii/Core Logic")
        if (phrutils::phr_failed(result)) return(result)
        ...
        result <- phrutils::phr_try_step({
          phrutils::phr_message(
            phrutils::phr_txt("Full FSL KII preset selection completed."),
            origin = phrutils::phr_txt("FSL KII Tool: Preset Full")
          )
        }, step = "mod_tools_fsl_kii_server/observeEvent_preset_full_fsl_kii/Result Handling")
        if (phrutils::phr_failed(result)) return(result)
    ...
    observeEvent(input$export_tool, {
      phrutils::phr_try({
        if (!isTRUE(protocol_r()$.tool_fsl_provider_kii)) {
          shiny::showModal(shiny::modalDialog(
            title = phrutils::phr_txt("Export FSL KII Tool"),
            phrutils::phr_txt("The FSL Service Provider KII tool has not been added to the protocol yet. Please add it from the Tool Design page before exporting."),
        ...
        shiny::showModal(shiny::modalDialog(
          title = phrutils::phr_txt("Export FSL KII Tool"),
          shiny::tagList(
            shiny::p(phrutils::phr_txt("Click below to save the FSL Service Provider KII tool as an Excel workbook with three sheets: revised_survey, revised_choices, and revised_settings.")),
      ...
      on_error = "warn",
      origin = phrutils::phr_txt("FSL KII Tool: Export"),
      hint = phrutils::phr_txt("Ensure the FSL Service Provider KII tool has been added to the protocol and exposes revised_survey / revised_choices / revised_settings.")
      )
    })

    output$download_tool <- shiny::downloadHandler(
      filename = function() {
        paste0("tool_kii_fsl_service_provider_iphra_v2",
      ...
      content = function(file) {
        ...
        sheets <- list(
          survey   = safe_df(protocol_r()$tools$tool_kii_fsl_service_provider_iphra_v2$revised_survey),
          choices  = safe_df(protocol_r()$tools$tool_kii_fsl_service_provider_iphra_v2$revised_choices),
          settings = safe_df(protocol_r()$tools$tool_kii_fsl_service_provider_iphra_v2$revised_settings)
        )

        writexl::write_xlsx(sheets, path = file)

        phrutils::phr_message(
          phrutils::phr_txt("FSL KII tool exported to Excel."),
          origin = phrutils::phr_txt("FSL KII Tool: Export")
        )
      }
    )
