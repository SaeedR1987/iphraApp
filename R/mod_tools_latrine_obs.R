  ...
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(input$preset_full_latrine_obs, {
      iphra_try({
        ...
        result <- iphra_try_step({
          iphra_message(
            iphra_txt("Latrine Observation preset selection completed."),
            origin = iphra_txt("Latrine Observation Tool: Preset Full")
          )
    ...
    observeEvent(input$export_tool, {
      iphra_try({
        if (!isTRUE(protocol_r()$.tool_latrine_observation)) {
          shiny::showModal(shiny::modalDialog(
            title = iphra_txt("Export Latrine Observation Tool"),
            iphra_txt("The Latrine Observation tool has not been added to the protocol yet. Please add it from the Tool Design page before exporting."),
        ...
        shiny::showModal(shiny::modalDialog(
          title = iphra_txt("Export Latrine Observation Tool"),
          shiny::tagList(
            shiny::p(iphra_txt("Click below to save the Latrine Observation tool as an Excel workbook with three sheets: revised_survey, revised_choices, and revised_settings.")),
      ...
      on_error = "warn",
      origin = iphra_txt("Latrine Observation Tool: Export"),
      hint = iphra_txt("Ensure the Latrine Observation tool has been added to the protocol and exposes revised_survey / revised_choices / revised_settings.")
      )
    })
    ...
    output$download_tool <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        iphra_message(
          iphra_txt("Latrine Observation tool exported to Excel."),
          origin = iphra_txt("Latrine Observation Tool: Export")
        )
      }
    )
